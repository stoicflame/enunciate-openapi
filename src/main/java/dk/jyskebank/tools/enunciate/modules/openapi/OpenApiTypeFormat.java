...
public class OpenApiTypeFormat {
	...
	private OpenApiTypeFormat(BaseType enunciateType, BaseTypeFormat enunciateFormat) {
		type = fromEnunciateType(enunciateType, enunciateFormat);
		format = toOpenApiFormat(enunciateFormat);
	}
	...
	private static OpenApiTypeFormat fromValidated(BaseType baseType, BaseTypeFormat format) {
	...
	public static OpenApiTypeFormat from(BaseType baseType, BaseTypeFormat format) {
	...
	private static String fromEnunciateType(BaseType baseType, BaseTypeFormat enunciateFormat) {
		switch (baseType) {
		case bool:
			return "boolean";
		case number:
			if (enunciateFormat != null && (BaseTypeFormat.INT32 == enunciateFormat || BaseTypeFormat.INT64 == enunciateFormat)) {
				return "integer";
			} else {
				return "number";
			}
