    ...
    private void computeResponses(EnunciateLogger logger) {
        ...
        if (method.getResponseCodes() != null) {
            for (StatusCode code : method.getResponseCodes()) {
                ...
                List<Parameter> headers = successResponse ? successHeaders : Collections.emptyList();
                Response response = new Response(logger, dataTypeReferenceRenderer, code.getCode(), headers, code.getCondition());

                if (code.getMediaTypes().isEmpty()) {
                    MediaAndType mediaAndType = FindBestDataTypeMethod.findBestMediaAndType(code.getMediaTypes());
                    DataTypeReference dataType = successResponse ? successDataType : null;
